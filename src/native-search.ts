...
interface SearchConfig {
  ...
  /** Explicit search provider override. When set, search/fetch always use this
   *  provider's backend instead of auto-detecting from the active model. */
...
//
// Compound provider names like "zai-anthropic" or "openai-codex" don't match
// the PROVIDERS map directly. resolveProvider() maps them to their base provider.
...
function normalizeConfig(value: unknown): SearchConfig {
  ...
  const providerOverrides =
    parsed.providerOverrides && typeof parsed.providerOverrides === "object"
      ? parsed.providerOverrides
      : {};
  const searchProvider =
    typeof parsed.searchProvider === "string" && parsed.searchProvider.trim()
      ? resolveProvider(parsed.searchProvider.trim())
      : undefined;
  return {
...
function loadConfig(): SearchConfig {
  try {
    ...
    if (existsSync(path)) return normalizeConfig(JSON.parse(readFileSync(path, "utf-8")));
  } catch {}
...
function getCurrentProvider(ctx: ExtensionContext) {
  ...
  return raw ? resolveProvider(raw) : raw;
}
function getSearchProvider(ctx: ExtensionContext, config: SearchConfig): string {
  const override = config.searchProvider?.trim();
...
export default function searchExtension(pi: ExtensionAPI) {
  ...
  pi.registerCommand("search", {
    ...
    getArgumentCompletions(p) {
      ...
      if (p.startsWith("provider ")) {
        ...
        return Object.keys(PROVIDERS)
          ...
          .map((id) => ({ value: `provider ${id}`, label: id }));
      }
      return staticArgs
    ...
    handler: async (args, ctx) => {
      ...
      if (sub?.startsWith("provider ")) {
        const chosen = sub.slice(9).trim();
        if (chosen === "auto" || chosen === "") {
          delete config.searchProvider;
          saveConfig(config);
          ctx.ui.notify("Search provider reset to auto-detect", "info");
        } else if (PROVIDERS[chosen] || PROVIDER_ALIASES[chosen]) {
          config.searchProvider = resolveProvider(chosen);
          saveConfig(config);
          ctx.ui.notify(`Search provider set to ${config.searchProvider}`, "info");
        } else {
          ctx.ui.notify(`Unknown provider: ${chosen}`, "warning");
        }
        updateStatus(ctx);
        return;
      }
  ...
  async function showSearchSettings(ctx: ExtensionContext) {
    await ctx.ui.custom((tui, theme, _kb, done) => {
      ...
      const activeSearchProvider = getSearchProvider(ctx, config);
      // Provider override selector — opens a submenu on Enter
      const providerIds = Object.keys(PROVIDERS).sort();
      const allProviderIds = ["auto", ...providerIds];
      items.push({
        id: "searchProvider",
        ...
        submenu: (_currentValue, done): Component => {
          const selectItems: SelectItem[] = allProviderIds.map((id) => {
            ...
            return {
              ...
              label:
                id === "auto"
                  ? "Auto-detect (follow active model)"
                  : `${cap!.name}${hasCredentials(id) ? " ✓" : " ✗"}`,
              description:
                id === "auto"
                  ? `Currently: ${PROVIDERS[activeSearchProvider]?.name ?? activeSearchProvider ?? "?"}`
      ...
      for (const pid of providerIds) {
        ...
        const current = pid === activeSearchProvider ? " ← active" : pid === currentProvider ? " ← current" : "";
        items.push({
      ...
      const settingsList = new SettingsList(
        ...
        (id, val) => {
          if (id === "enabled") config.enabled = val === "enabled";
            ...
            config.searchProvider = val === "auto" ? undefined : val;
          }
